...
namespace Emscripten.Build.CPPTasks
{
    internal class Utils
    {
        ...
        private static readonly string[] EMSCRIPTEN_REGEX_MATCH = {
        ...
        private static readonly string[] EMSCRIPTEN_REGEX_FILENAME = {
                                                 @"$1",
                                                 @"$1",
                                                 @"$1",
                                                 @"$2",
        ...
        private static readonly string[] EMSCRIPTEN_REGEX_REPLACE = {
        ...
        public static string EmscriptenOutputReplace(string line)
        {
            // Replaces given Emscripten style output, with output that obeys Visual Studio's 'jump to line' formatting. So:
            ...
            for (int i = 0; i < EMSCRIPTEN_REGEX_MATCH.Length; i++)
            {
                // Do we match this...?
                Regex regEx = new Regex(EMSCRIPTEN_REGEX_MATCH[i]);
                if (regEx.IsMatch(line))
                {
                    // Good, we do. For now just grab the filename portion
                    string filename = regEx.Replace(line, EMSCRIPTEN_REGEX_FILENAME[i]);
                    ...
                    try
                    {
                        ...
                        string lastBit = regEx.Replace(line, EMSCRIPTEN_REGEX_REPLACE[i]);
